...
{
  // Make a lowercase copy so the checks are not case-sensitive.
  std::string local_sql(sql);
  std::transform(local_sql.begin(), local_sql.end(), local_sql.begin(),
    [](unsigned char c) { return static_cast<char>(std::tolower(c)); });

  // The supplied attack code adds conditions such as:
  //   OR 1=1
  //   OR 2=2
  //   OR 'hi'='hi'
  // Detect the general pattern instead of checking for one hard-coded value.
  const std::regex always_true_or(
    R"(\bor\s+((?:'[^']*')|(?:"[^"]*")|(?:[a-z0-9_]+))\s*=\s*\1)",
    std::regex_constants::icase);

  const bool has_tautology = std::regex_search(local_sql, always_true_or);
  const bool has_comment = local_sql.find("--") != std::string::npos ||
                           local_sql.find("/*") != std::string::npos;
  const bool has_union = local_sql.find(" union select ") != std::string::npos;

  if (has_tautology || has_comment || has_union)
  {
    records.clear();
    std::cout << "Suspected SQL injection detected. Query blocked." << std::endl;
    return false;
  }
